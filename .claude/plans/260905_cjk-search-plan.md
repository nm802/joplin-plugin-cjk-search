...
この分離で、保護のためのテスト（T-1〜T-16）を Joplin デスクトップ無しに RPi 上で回せる。

**アナライザは既定 `cjk`（Lucene CJKAnalyzer 互換）、オプション `full-bigram`（独自）。** 公開時に「よく知られた構成」として説明できることを優先する。Lucene 本体は Java でプラグインからは使えないため、移植ではなく同一構成の再実装であることを README で明示する。切り替えは索引の全再構築を伴うため、`meta` テーブルに記録して起動時に検知し、確認ダイアログを出す。
...
- [ ] Analyzer インタフェースと2実装（`cjk` = Lucene CJKAnalyzer 互換 / `full-bigram` = 独自）
- [ ] 索引構築・クエリ実行の実装（FTS4 + bigram 展開、1文字は前方一致、末尾文字は unigram）
- [ ] 索引 `meta` テーブルとアナライザ変更検知・再構築確認
- [ ] 保護のためのテスト T-1〜T-16 を実装（先に Red を確認してから通す）
...
- 索引サイズ。500ノート・2.2M字の実測で `full-bigram` 21MB（本文の9.5倍）、`cjk` 17MB（7.9倍）。2750ノート実データでの計測が必要
- スコア式
